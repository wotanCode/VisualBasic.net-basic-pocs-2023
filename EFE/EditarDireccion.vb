...
Public Class EditarDireccion
    ' Método para actualizar la dirección en la base de datos
    Private Sub ActualizarDireccionEnBD(rut As String, regionPersonal As String, comunaPersonal As String, callePersonal As String,
                                    regionComercial As String, comunaComercial As String, calleComercial As String,
                                    regionFacturacion As String, comunaFacturacion As String, calleFacturacion As String)
        Try
            ' Abrir la conexión a la base de datos
            abrirConexionBBDD()

            ' Modificar la consulta para actualizar las direcciones
            Dim query As String = "UPDATE Persona SET direccionPersonal = @direccionPersonal, direccionComercial = @direccionComercial, direccionFacturacion = @direccionFacturacion WHERE rut = @rut"
            ...
            Using cmd As New SqlCommand(query, conexion)
                ...
                MessageBox.Show("Direcciones actualizadas correctamente en la base de datos.")
            End Using
        Catch ex As Exception
            ' Mostrar un mensaje de error si ocurre alguna excepción
            MessageBox.Show("Error al actualizar las direcciones en la base de datos. Detalles del error: " & ex.Message)
        Finally
    ...
    Private Sub Button_modificar_Click(sender As Object, e As EventArgs) Handles Button_modificar.Click
        ...
        ' Obtener las regiones, comunas y calles ingresadas por el usuario
        Dim regionPersonal As String = ComboBox1_regionPersonal.Text
        Dim comunaPersonal As String = ComboBox2_comunaPersonal.Text
        Dim callePersonal As String = TextBox12_callePersonal.Text

        Dim regionComercial As String = ComboBox4_regionComercial.Text.Trim()
        Dim comunaComercial As String = ComboBox3_comunaComercial.Text.Trim()
        Dim calleComercial As String = TextBox9_calleComercial.Text

        Dim regionFacturacion As String = ComboBox6_regionFacturacion.Text.Trim()
        Dim comunaFacturacion As String = ComboBox5_comunaFacturacion.Text.Trim()
        Dim calleFacturacion As String = TextBox10_calleFacturacion.Text

        ' Llamar al método para actualizar las direcciones en la base de datos
        ActualizarDireccionEnBD(rutSeleccionado, regionPersonal, comunaPersonal, callePersonal,
                                regionComercial, comunaComercial, calleComercial,
                                regionFacturacion, comunaFacturacion, calleFacturacion)
    End Sub
